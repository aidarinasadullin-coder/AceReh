...
| V4 | Волна 3 (тесты) | **(б)** отложить… — **пересмотрено владельцем 2026-09-18 («Добиваем»)**: волна 3 выполнена тем же циклом; см. §3 |
...
5. Волна 3 — по решению владельца (V4; пересмотрено 2026-09-18 —
   выполнена): `HydraulicsIntegrationTestBase` (канон Thermal; Glycol —
   без PipeSpacing-бэкинга, WriterAuthority — сводка по TotalLength/
   IsValid, оба расхождения зафиксированы в override'ах),
   `CalculationReportTestProjectFactory` (расхождение DataBuilder —
   Throttling/ValveTurns уровня контура — флаг
   `withCircuitLevelThrottling`), `PdfImageStreamValidator` (using static).
