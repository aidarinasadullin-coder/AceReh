...
Факт имплементации 2026-09-18: builder — `tests/Fixtures/ProjectDataBuilder.cs`
(118 строк). Стартует пустым (эквивалент `new ProjectData()`), слайсы
создаются лениво — это сохраняет wire-характеристику намеренно частичных
инлайнов `ProjectRoundTripTests` (FlowRegime, BackwardCompatible и др. с
null-слайсами); `WithDefaultSlices()` воспроизводит явную инициализацию
четырёх слайсов ResultsViewModel-семейства. Переведены 24
инициализатор-инлайна (13 в split-файлах + 11 в `ProjectRoundTripTests`);
6 голых `new ProjectData()` в мок-стабах промпт-тестов — не fixture-конструкции
и не переводились (уточнение к V4(a): исходный grep-счёт «19» включал их).
Пины волны: 0 остаточных инициализатор-инлайнов у двух потребителей;
assert-счётчики затронутых файлов без изменений (13/64/95/30, PRT 145);
полный набор 2300/2300 зелёный.
