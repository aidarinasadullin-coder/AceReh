...
- **Основной риск:** уже dirty central files и размножение events/calculations при замене owner; риск ограничивается NUL-safe baseline, preimage/hunk receipts, exact multiplicity tests и запретом параллельных central edits.
...
Этот `.omo` artifact является reviewed **planning candidate**, а не active architecture execution plan. Текущий authoritative `docs/architecture-migration/STATE.json` остаётся на completed Phase 3.1 с `stop=true`; поэтому никакая строка ниже не исполняется из текущего state. Перед freeze/import/review repository workflow обязан создать отдельную Phase 4 planning identity, materialize canonical mirror path/SHA и пройти `/architecture-approve phase-4-thermal-state`; execution затем требует отдельного `/architecture-start phase-4-thermal-state`. Если canonical workflow transition не выполнен или external Boulder mismatch блокирует hook, workflow останавливается — worker не запускает Todo 1 и не редактирует `STATE.json`/Boulder вручную.

## Scope

### In scope

- Current repository only: `D:\IA\ace v.2`, с execution-time verified Git root/HEAD/branch и fresh dirty baseline.
...
- No stage/commit/reset/revert/checkout/restore/clean/push of unrelated worktree paths. Commit lines below are guidance for an explicitly authorized isolated worker only.
...
- Recovery uses a minimal inverse patch for worker-owned hunks or restores a task-created file from its recorded preimage. Never use Git reset/checkout/restore/clean in the shared dirty worktree.
...
All commands run from `D:\IA\ace v.2` in PowerShell 7 (`pwsh`) and preserve `$LASTEXITCODE`. Every todo below names one or more command IDs; that reference is the exact tool/invocation, fixture and expected artifact for its QA.
...
dotnet test "tests/SnowMeltingCalculator.Tests/SnowMeltingCalculator.Tests.csproj" -c Release --no-build --filter "FullyQualifiedName~ProjectSessionThermalStateTests|FullyQualifiedName~ThermalMultiplicityCharacterizationTests|FullyQualifiedName~ThermalStateLegacyStoreGuardTests|FullyQualifiedName~ThermalViewModelTests|FullyQualifiedName~CalculationStateServiceTests|FullyQualifiedName~DiRegistrationTests" --logger "trx;LogFileName=phase-4-focused.trx" --results-directory "tests/SnowMeltingCalculator.Tests/TestResults"

# V3 — upstream invalidation; expect exit 0, failed=0 and exact Phase 3/3.1 count assertions.
dotnet test "tests/SnowMeltingCalculator.Tests/SnowMeltingCalculator.Tests.csproj" -c Release --no-build --filter "FullyQualifiedName~ClimateThermalInvalidationRegressionTests|FullyQualifiedName~ConstructionThermalInvalidationRegressionTests" --logger "trx;LogFileName=phase-4-upstream-invalidation.trx" --results-directory "tests/SnowMeltingCalculator.Tests/TestResults"

# V4 — context/Hydraulics consumers; expect exit 0, failed=0 and exact count/order assertions.
dotnet test "tests/SnowMeltingCalculator.Tests/SnowMeltingCalculator.Tests.csproj" -c Release --no-build --filter "FullyQualifiedName~ThermalToHydraulicsIntegrationTests|FullyQualifiedName~PipeSpacingSynchronizationTests|FullyQualifiedName~DoubleCalculationPreventionTests|FullyQualifiedName~CalculationContextInvalidationTests|FullyQualifiedName~CalculationContextWriterAuthorityTests" --logger "trx;LogFileName=phase-4-hydraulics-consumer.trx" --results-directory "tests/SnowMeltingCalculator.Tests/TestResults"

# V5 — lifecycle/persistence/Results; expect exit 0, failed=0 and all mapper/lifecycle fixtures executed.
dotnet test "tests/SnowMeltingCalculator.Tests/SnowMeltingCalculator.Tests.csproj" -c Release --no-build --filter "FullyQualifiedName~ProjectRoundTripTests|FullyQualifiedName~ResultsViewModelOpenProjectTests|FullyQualifiedName~ProjectLifecycleFlowCharacterizationTests|FullyQualifiedName~ThermalPersistenceMapperTests" --logger "trx;LogFileName=phase-4-persistence.trx" --results-directory "tests/SnowMeltingCalculator.Tests/TestResults"

# V6 — full Release; expect exit 0, failed=0, no new exact NotExecuted identity vs baseline.
dotnet test "tests/SnowMeltingCalculator.Tests/SnowMeltingCalculator.Tests.csproj" -c Release --no-build --logger "trx;LogFileName=phase-4-full-release.trx" --results-directory "tests/SnowMeltingCalculator.Tests/TestResults"
...
# V9 — prepare deterministic task-owned .smc inputs, then run the frozen WPF executable through the UI QA harness.
# Each command must exit 0; harness starts/owns/closes the process and writes JSON/screenshots/logs.
pwsh -NoProfile -File "docs/architecture-migration/evidence/phase-4-thermal-state/prepare-ui-fixtures.ps1" -Source "tests/SnowMeltingCalculator.Tests/Fixtures/v1-sample.smc" -OutputDirectory "docs/architecture-migration/evidence/phase-4-thermal-state/fixtures"
pwsh -NoProfile -File "docs/architecture-migration/evidence/phase-4-thermal-state/run-wpf-ui-qa.ps1" -Executable "src/bin/Release/net8.0-windows/win-x64/SnowMeltingCalculator.exe" -ExpectedExecutableSha256File "docs/architecture-migration/evidence/phase-4-thermal-state/frozen-release-sha256.json" -ProjectA "docs/architecture-migration/evidence/phase-4-thermal-state/fixtures/project-a.smc" -ProjectB "docs/architecture-migration/evidence/phase-4-thermal-state/fixtures/project-b.smc" -InvalidProject "docs/architecture-migration/evidence/phase-4-thermal-state/fixtures/unknown-pipe.smc" -OutputDirectory "docs/architecture-migration/evidence/phase-4-thermal-state/ui-qa"

# V10 — structural plan/task grammar; expect no output and exit 0.
$plan='.omo/plans/phase-4-thermal-state.md'
$bad=Select-String -Path $plan -Pattern '^\s+- \[ \] (?:\d+|F\d+)\.' | Where-Object { $_.Line -notmatch '^- \[ \] (?:[1-9]\d*|F[1-9]\d*)\. ' }
if ($bad) { $bad | ForEach-Object { $_.Line }; exit 1 }

# V11 — first available in Todo 11 after ThermalStateLegacyStoreGuardTests is created; unavailable and forbidden in Todos 1-10.
# Its named NUnit negative fixtures own repository-wide source guards and never mutate source; expect exit 0 because each test asserts the guard rejects its fixture.
dotnet test "tests/SnowMeltingCalculator.Tests/SnowMeltingCalculator.Tests.csproj" -c Release --no-build --filter "FullyQualifiedName~ThermalStateLegacyStoreGuardTests&TestCategory=NegativeFixture" --logger "trx;LogFileName=phase-4-negative-fixtures.trx" --results-directory "tests/SnowMeltingCalculator.Tests/TestResults"
...
# V12-F4 — F4-owned model/runtime receipts; does not overwrite Todo 14 canonical receipts.
node "docs/architecture-migration/widget/verify-widget.mjs" --suite model-v2 --schema "docs/architecture-migration/maps/architecture-model.widget.schema.json" --model "docs/architecture-migration/maps/architecture-model.json" --output "docs/architecture-migration/evidence/phase-4-thermal-state/final/f4/model-v2.json"
node "docs/architecture-migration/widget/verify-widget.mjs" --suite runtime-v2 --schema "docs/architecture-migration/maps/architecture-model.widget.schema.json" --model "docs/architecture-migration/maps/architecture-model.json" --output "docs/architecture-migration/evidence/phase-4-thermal-state/final/f4/runtime-v2.json"
...
**Widget browser contract (exact agent-executable browser tool):** use the loaded `playwright` skill MCP, not a repository package or an unspecified harness. Invoke `browser_navigate` with the absolute `file:///D:/IA/ace%20v.2/docs/architecture-migration/architecture-widget.html`; then for each ID in `compile-time`, `di-runtime`, `state-ownership`, `reactive`, `persistence`, `user-flow`, invoke `browser_click` with selector `[data-view="<ID>"] button`, invoke `browser_evaluate` to assert that this button has `aria-pressed="true"`, `[data-field="state-kind"]` is neither empty nor `error`, and `[data-result-rows] tr` has a positive count; invoke `browser_take_screenshot` (`scale: "css"`, `fullPage: true`). Todo 14 uses filename `phase-4-widget-<ID>.png`; F4 uses the isolated filename `f4-phase-4-widget-<ID>.png`. Finally invoke `browser_console_messages` at level `error` and require zero messages, then `browser_close`. Expected artifacts are the six owner-specific `.playwright-mcp/<owner-prefix>phase-4-widget-<ID>.png` files plus the per-ID assertion/count table copied into the owning evidence Markdown. A missing selector, zero rows, error state, console error, or missing screenshot is a hard failure.

TRX reconciliation is performed by the repository's existing test-result parser if present; otherwise Todo 1 creates test-only `parse-trx.ps1` and `assert-trx-identities.ps1` under the Phase 4 evidence directory. The first reads XML and emits exact `test-case-name/outcome` JSON; the second accepts exactly one TRX, one immutable expected-identity manifest and one group, then rejects a missing file, zero tests, duplicate identities, non-Passed outcomes, absent expected identities, or unexpected identities. Todo 2 writes `expected-negative-test-identities.json` from the exact fully-qualified names of the characterized `CalculationFailure`, `PersistenceFailure`, and `RestoreFailure` cases, with each group non-empty and disjoint. Both scripts are locked by task-owned XML/manifest fixtures for Passed/Failed/NotExecuted/zero-test/missing/duplicate/unexpected cases. The general parser command is `pwsh -File "docs/architecture-migration/evidence/phase-4-thermal-state/parse-trx.ps1" -InputDirectory "tests/SnowMeltingCalculator.Tests/TestResults" -Output "docs/architecture-migration/evidence/phase-4-thermal-state/trx-identities.json"`; V12-F3 separately reconciles each negative TRX before any UI QA, so the full-suite TRX cannot satisfy a filtered lane.

**Frozen Release verifier contract:** Todo 12 creates `verify-frozen-release.ps1` beside `frozen-release-sha256.json`. The manifest has exactly four canonical workspace-relative keys: `executable`, `productDll`, `testDll`, and `plan`, each with path and uppercase SHA-256. The verifier rejects extra/missing keys, missing/non-regular files, path escape, duplicate resolved paths, or any hash mismatch; on success it writes a lane-owned JSON receipt containing manifest SHA-256 plus the same four resolved paths/hashes. F1, F2, F3 and F4 each invoke V13 immediately before and after their own commands, writing `frozen-hashes-before.json` and `frozen-hashes-after.json` inside that lane's directory, and require byte-identical four-hash sets. Their Markdown receipts must echo all four hashes and the manifest hash. F5 rejects a missing receipt, differing before/after values, or any cross-lane mismatch. No final lane rewrites the manifest or frozen binaries.

## Execution strategy

V11 is forbidden in Todos 1-10. It first becomes available in Todo 11, where the newly created `ThermalStateLegacyStoreGuardTests` suite owns repository-wide source guards; earlier todos use only task-local tests within their existing allow-lists.
...
- [ ] 1. Capture protected Phase 4 baseline, tools, plan identity and dirty preimages
  ...
  - **Allow-list:** new files under `docs/architecture-migration/evidence/phase-4-thermal-state/`; factual workflow state/history fields only through canonical workflow commands.
  - **References:** `docs/architecture-migration/AGENTS.md`; `STATE.json`; current 281-entry audit is only a planning fact and must be freshly recalculated.
  - **Action:** record Git root/HEAD/branch/upstream, `dotnet --info`, `node --version`, binary `git status --porcelain=v1 -z --branch`, staged/unstaged/untracked NUL-safe sets, baseline builds/tests and exact accepted `NotExecuted` identities. For every intended pre-dirty path, store SHA-256 and exact preimage/hunk receipt; define protected set and task-owned evidence set.
  - **Acceptance:** fresh state validation and plan SHA pass; baseline commands/exits/counters/hashes are reproducible; staged set is unchanged; every dirty intended file has a preimage/hunk receipt; immediate post-capture symmetric comparison differs only by Todo 1 evidence.
  - **QA — happy:** run V0, V1 and the Todo 1 NUL-safe capture script twice without source edits; parsers produce identical protected sets/hashes and V1 logs become the warning/NotExecuted baseline. Save `task-1-baseline.md` plus raw binary/status/build/TRX files; expect every command exit 0.
  - **QA — failure:** run the capture parser's task-owned fixture containing spaces/non-ASCII/NUL boundaries and the TRX parser's zero-test fixture; each test expects the parser to retain identity or reject zero execution. Any HEAD/staged/protected drift blocks Todo 2 and is recorded, never reset.
  - **Commit:** `test(architecture): capture phase 4 thermal baseline`.

### Mandatory baseline correction before Todo 2

The superseded Phase 4 plan identity of `93512` bytes with SHA-256 `C238E2876B43F8D606F24DF9682CAFD41F9B71C6C2DDDF965814C99EEC8AD451` remains historical provenance. Todo 1 baseline investigation identified a pre-existing test-compilation defect relative to provenance base `e655735dfa66c00cf9c53be93d511eda8989e8bf`: three `ResultsViewModel` constructor call sites omit the already-required `HydraulicSummaryBuilder` argument and therefore produce `CS7036` before any Phase 4 Thermal behavior can execute.

After separate owner approval and execution authorization for this amended plan, Todo 1 must apply exactly three constructor-argument additions, using the existing test construction pattern, at these call sites and no others:

1. `tests/SnowMeltingCalculator.Tests/Construction/ConstructionServiceTests.cs:1154`
2. `tests/SnowMeltingCalculator.Tests/Construction/ConstructionServiceTests.cs:1221`
3. `tests/SnowMeltingCalculator.Tests/Services/Navigation/DialogServiceThreadAffinityTests.cs:136`

The correction source allow-list is exactly the two test files named above. It may add only the three required `HydraulicSummaryBuilder` arguments and must not change assertions, test logic, production constructors, production behavior, runtime wiring, any other test path, Thermal implementation, maps, widget, `AGENTS.md`, Boulder, or unrelated dirty paths. The supplied planning evidence is provenance input only and must not be copied, renamed, inferred, reused, or treated as executable evidence. New correction and baseline evidence may be written only under `docs/architecture-migration/evidence/phase-4-thermal-state/` by the authorized execution lane.

Before the correction, Todo 1 captures the exact preimage and hunk receipt against the base-relative dirty baseline. After the correction, Todo 1 must, in order: build production Debug and Release with exit `0`; build the test project Debug and Release with exit `0`; run full Debug and Release tests into newly generated real TRX files; reconcile both TRX files by exact test identity and outcome; and prove through a fresh symmetric dirty-baseline comparison that only the approved three test hunks and task-owned evidence differ. Copied, renamed, inferred, reused or missing TRX is forbidden. Any build or test failure, unexpected `NotExecuted` identity, identity drift, or allow-list drift keeps `BASELINE_EXECUTABLE` blocked.

Todo 2 is prohibited until both test configurations build, both real TRX files exist and reconcile, the correction is proven test-only and pre-existing at base `e655735dfa66c00cf9c53be93d511eda8989e8bf`, the exact two-file allow-list is preserved, no protected path differs outside the approved three hunks and task-owned evidence, and the authorized execution lane updates the Todo 1 baseline receipt. Failure of any condition stops in Todo 1 without widening scope.

Rollback is one three-hunk boundary: revert only the two `ConstructionServiceTests.cs` argument-addition hunks and the one `DialogServiceThreadAffinityTests.cs` argument-addition hunk from their recorded preimages. Never use `git reset`, `git checkout`, `git restore`, `clean`, or any operation that changes unrelated dirty paths; production files and unrelated paths are never rollback targets.

- [ ] 2. Lock Thermal writers, subscribers, calculations, lifecycle and persistence behavior before ownership edits
  - **Depends on:** Todo 1, including the mandatory baseline correction and every post-correction build/test/TRX/identity/allow-list gate above, all green; otherwise Todo 2 remains forbidden.
  ...
  - **QA — happy:** run `dotnet test "tests/SnowMeltingCalculator.Tests/SnowMeltingCalculator.Tests.csproj" -c Debug --filter "FullyQualifiedName~ThermalMultiplicityCharacterizationTests" --logger "trx;LogFileName=phase-4-characterization-debug.trx" --results-directory "tests/SnowMeltingCalculator.Tests/TestResults"` and V3; expect exit 0, failed 0, every matrix case executed, and emit `task-2-thermal-characterization.md`.
  - **QA — failure:** task-local cases in `ThermalMultiplicityCharacterizationTests` model one synthetic direct writer and duplicate subscriber and pass only by asserting both violations are rejected. No production file is mutated, and V11 is unavailable before Todo 11.
...
- [ ] 3. Add immutable Thermal state contract, structural equality and direct state tests
  ...
  - **QA — happy:** run `dotnet test "tests/SnowMeltingCalculator.Tests/SnowMeltingCalculator.Tests.csproj" -c Debug --filter "FullyQualifiedName~ProjectSessionThermalStateTests" --logger "trx;LogFileName=phase-4-state-debug.trx" --results-directory "tests/SnowMeltingCalculator.Tests/TestResults"`; expect exit 0/failed 0 and all mutation/origin cases executed. Evidence: `task-3-thermal-state-contract.md`.
...
- [ ] 4. Attach exactly one ThermalState to ProjectSession and prove runtime DI identity
  ...
  - **QA — happy:** run `dotnet test "tests/SnowMeltingCalculator.Tests/SnowMeltingCalculator.Tests.csproj" -c Debug --filter "FullyQualifiedName~DiRegistrationTests&TestCategory=ThermalState" --logger "trx;LogFileName=phase-4-di-debug.trx" --results-directory "tests/SnowMeltingCalculator.Tests/TestResults"`; expect exit 0 and reference-identity/disposal cases executed. Evidence: `task-4-project-session-di.md`.
  - **QA — failure:** task-local negative cases in the Todo 4 DI/ProjectSession test allow-list model duplicate registration and a constructor cycle; they pass only if the tests reject the duplicate/cycle without service location. V11 is unavailable before Todo 11.
...
- [ ] 5. Convert CalculationStateService Thermal status and spacing into a compatibility adapter
  ...
  - **QA — happy:** run `dotnet test "tests/SnowMeltingCalculator.Tests/SnowMeltingCalculator.Tests.csproj" -c Debug --filter "FullyQualifiedName~CalculationStateServiceTests|FullyQualifiedName~ThermalMultiplicityCharacterizationTests" --logger "trx;LogFileName=phase-4-status-spacing-debug.trx" --results-directory "tests/SnowMeltingCalculator.Tests/TestResults"`; expect exit 0/failed 0 and exact status/spacing event counts. Evidence: `task-5-calculation-state-adapter.md`.
  - **QA — failure:** task-local negative cases in the Todo 5 direct service/state/spacing test allow-list exercise noncanonical writes and duplicate completions; they pass only when direct writes are rejected/routed and event counts remain at most one. V11 is unavailable before Todo 11.
...
- [ ] 6. Make ThermalViewModel a canonical-state adapter and move calculation orchestration out of writable UI fields
  - **Depends on:** Todo 5.
  - **Allow-list:** `ThermalViewModel.cs`; new `src/Services/Project/IThermalStateCoordinator.cs` and `src/Services/Project/ThermalStateCoordinator.cs`; the exact coordinator registration/composition seam in `src/Configuration/ServiceCollectionExtensions.cs`; `src/Views/Thermal/ThermalView.xaml`, `src/Views/Hydraulics/CircuitsView.xaml`, and `src/Views/Results/ResultsView.xaml` only for the exact accessibility-only AutomationIds named by the V9 contract; direct Thermal VM/coordinator/calculation/DI and selector-contract tests; minimal constructor helpers; Phase 4 evidence.
...
- [ ] 7. Move Climate and Construction invalidation to one canonical Thermal subscriber
  ...
  - **QA — failure:** V3 includes changed `ProjectLoadReset` zero-event cases; a task-local duplicate-subscriber case in the Todo 7 coordinator/VM or directly affected regression test allow-list passes only by proving multiplicity is detected. V11 is unavailable before Todo 11.
...
- [ ] 8. Publish one Thermal projection through CalculationContext and preserve Hydraulics/spacing consumer counts
  ...
  - **QA — failure:** V4's invalid/null/no-op task-local cases assert calculator zero; a task-local second-writer case in the Todo 8 related integration/authority test allow-list passes only if the authority test rejects it. V11 is unavailable before Todo 11.
...
- [ ] 9. Route lifecycle reset, project restore and fallback calculation through ThermalState
  ...
  - **QA — happy:** run V1, then `dotnet test "tests/SnowMeltingCalculator.Tests/SnowMeltingCalculator.Tests.csproj" -c Release --no-build --filter "FullyQualifiedName~ProjectLifecycleFlowCharacterizationTests|FullyQualifiedName~ClimateThermalInvalidationRegressionTests|FullyQualifiedName~ResultsViewModelOpenProjectTests" --logger "trx;LogFileName=phase-4-lifecycle.trx" --results-directory "tests/SnowMeltingCalculator.Tests/TestResults"`; V1 recompiles all Todo 9 changes before `--no-build`; expect exit 0/failed 0 and all DEC-T08 rows executed. Save `task-9-lifecycle-restore.md`.
...
- [ ] 11. Enforce final sole-owner, immutable projection, subscription and DI guards
  ...
  - **Action:** make guards fail on writable Thermal VM fields, Thermal/spacing service fields, direct orchestrator assignment, Results save from VM/service spacing, unapproved `UpdateThermal*`, mutable snapshot references, duplicate subscriber and independent DI state registration. Do not edit production code in this todo: any detected legacy seam returns to the owning earlier todo, invalidates downstream receipts, and is corrected only under that todo's exact allow-list and gates.
  ...
  - **QA — happy:** after creating `ThermalStateLegacyStoreGuardTests`, run V1, then V2, then V11, and emit the guard suite's machine-readable writer/subscriber graph at `task-11-ownership-guards.md`; V1 recompiles all Todo 11 changes before V2 and V11 use `--no-build`; expect every command exit 0/failed 0.
  - **QA — failure:** run V11; every in-memory/source-string violation category must execute and pass only by matching its intended rejected symbol/path.
...
- [ ] 12. Run Debug/Release builds, focused/affected/full suites and reconcile exact executable evidence
  - **Depends on:** Todo 11.
  - **Allow-list:** only raw logs, TRX, `parse-trx.ps1`, `assert-trx-identities.ps1`, `verify-frozen-release.ps1`, their task-owned fixtures/manifests, `frozen-release-sha256.json`, and receipts under `docs/architecture-migration/evidence/phase-4-thermal-state/`. No production, test-source, map, model, widget, or dossier edits. A gate failure returns to the exact owning Todo 2-11 while preserving the failure receipt; if no owning allow-list covers the required correction, stop and re-plan.
  - **References:** Todo 1 baseline commands and exact known `NotExecuted`; verification strategy filters.
  - **Action:** run Debug and Release builds; focused Thermal/state/guard; upstream invalidation; Hydraulics consumer; lifecycle/persistence/Results; then full Release. Parse every TRX by exact identity and compare warnings/errors/failed/skipped/NotExecuted with baseline.
  ...
  - **QA — happy:** run V1 → V2 → V3 → V4 → V5 → V6 and the exact TRX parser command; expect every exit 0 and stable non-empty identity sets. Evidence: `task-12-executable-gates.md` plus raw files.
  - **QA — failure:** run the parser's zero-test/missing-TRX/stale-run fixtures; parser must exit nonzero. Any mismatched `NotExecuted` identity is a hard blocker even when `dotnet test` exited 0.
...
- [ ] 14. Refresh all six architecture views, shared model, widget and workflow evidence
  ...
  - **Action:** mark ThermalState sole owner, adapters/projections/origins/completions, removed writers, persistence compatibility and executable evidence in all affected views. Update one shared model; run model-v2/runtime-v2; generate widget twice; run `--check`; update workflow only to final-verification/awaiting-owner-acceptance as appropriate—never completed before acceptance.
  ...
  - **QA — happy:** run V0, V7 and V8; then execute the exact Playwright MCP widget browser contract for all six IDs. Expect all commands/exits/assertions green, equal generation hashes, zero browser console errors and six `.playwright-mcp/phase-4-widget-<ID>.png` artifacts; record the assertion/count table in `task-14-architecture-dossier.md`.
...
- [ ] F1. Verify Conformance / Scope / Provenance, dirty-worktree preservation and architecture-dossier fidelity
  ...
  - **Action:** run V13 into `final/f1/frozen-hashes-before.json`; independently map every changed path/hunk to one todo allow-list, DEC-T01..T08 and scope; verify owner gates, canonical/mirror plan identity, evidence provenance, unchanged staged set and no lost unrelated user hunk. Run model-v2/runtime-v2 and inspect all six filters/shared IDs/edges without regenerating canonical artifacts; verify the canonical widget hash remains Todo 14's deterministic hash and workflow is only awaiting owner acceptance, not authorizing Phase 5. Then run V13 into `final/f1/frozen-hashes-after.json`.
  ...
  - **QA — happy:** run V13-before, V0, V10, `pwsh -File "docs/architecture-migration/evidence/phase-4-thermal-state/verify-protected-baseline.ps1" -Baseline "docs/architecture-migration/evidence/phase-4-thermal-state/baseline-git-status.bin" -Output "docs/architecture-migration/evidence/phase-4-thermal-state/final/f1/protected.json"`, V12-F4, verify the canonical widget hash recorded by Todo 14, and execute the exact Playwright MCP widget browser contract without V8 regeneration using screenshot names `f1-phase-4-widget-<ID>.png`; then run V13-after. Expect exits 0, `protected_mismatch_count=0`, F1-owned model/runtime receipts, six screenshots, zero console errors and current workflow only awaiting owner acceptance. Produce `final/f1/conformance-scope-provenance.md` with exact path/hunk matrix, per-ID assertion/count table, manifest hash, all four frozen hashes and APPROVE/REJECT.
...
- [ ] F4. Consolidate the three immutable final-domain receipts without overriding any verdict
  ...
  - **Action:** read all three domain receipts and their machine artifacts; verify each says APPROVE, each echoes identical executable/product/test DLL/plan hashes, every referenced artifact exists and matches its recorded SHA, and list reused versus independently rerun evidence plus residual risks. Write only `final/consolidated/final-receipt.md` and `final/consolidated/artifact-manifest.json`. This step has no authority to reinterpret, waive or override a domain rejection.
  ...
  - **QA — happy:** run the Todo 1 manifest verifier against all three receipt directories and `frozen-release-sha256.json`; expect exit 0, three APPROVE inputs, one hash identity and complete artifact coverage. Produce the two consolidated artifacts with APPROVE.
...
- During separately authorized execution, use one green atomic commit per Todo 1-14 only when the worker is in an approved isolated branch/worktree or receives explicit commit permission.
