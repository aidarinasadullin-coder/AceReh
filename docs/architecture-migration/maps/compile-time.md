...
`CTE-005..CTE-008` (orchestrator constructor references to the four module ViewModels) closed: the constructor now takes `IProjectLoadClimateAdapter`/`IProjectLoadConstructionAdapter`/`IProjectLoadThermalAdapter`/`IProjectLoadHydraulicsAdapter` (`ProjectRestoreAdapters.cs`, application-owned). `ResultsPdfDataBuilder` takes `IReportConstructionLayerSource`/`IReportCollectorDataSource`. The dead legacy loader `LoadLayersFromProjectDataLegacy` was removed (0 callers; its direct VM collection writes are gone — guard suite re-pinned). `ResultsViewModel` ctor lost the `IProjectStateService`/`IMarkDirtyService` parameters (session-only surface). `IProjectStateService`/`IProjectInfoService` types deleted. Static proof: `ApplicationServiceViewModelDecouplingTests`. Evidence: `slice-4..slice-6` receipts; model records `EV-P9-SLICE-4..6`.
