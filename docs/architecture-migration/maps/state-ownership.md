...
Accepted Phase 9 result: the named Phase 8 residuals are closed. `Results.Circuits` holds Results-owned `CircuitRow` copies reconstructed from canonical `HydraulicsState` snapshots (`HydraulicCircuitRowProjection`); the module rows are neither referenced nor mutated (ownership negative probes, slice-3). `UpdateCollectorSummary` builds `CollectorSummary` from `HydraulicCollectorSummarySnapshot` by the Results selection (ST-027); `HydraulicSummaryBuilder` consumes canonical snapshots only (ST-026). `ResultsViewModel` holds no concrete module-ViewModel reference. `ProjectLoadOrchestrator` depends on application-owned adapter interfaces (`IProjectLoad*Adapter`, `src/Services/Project/ProjectRestoreAdapters.cs`), not concrete VMs; `ResultsPdfDataBuilder` reads via `IReport*Source` bound to the same singletons (report content unchanged). Legacy forwarding aliases `IProjectStateService`/`IProjectInfoService` and the legacy `ProjectStateService` class are removed from production (test support copy under `tests/.../Fixtures/`); consumers use `IProjectSession`. `IMarkDirtyService` is retained as the internal session dirty seam only (`ProjectSession : IMarkDirtyService`; DI registers the session itself; module adapters and the thermal coordinator receive it) — plan premise "dead params" was disproven by live code (Climate/Construction VMs pass it into session-slice ctors; ThermalViewModel passes it into the coordinator), recorded as a slice-6 deviation. LIM-P8-2 resolved by owner decision B: restore is import-less (read-only catalogs on open); custom entries stay project-local. Evidence: `slice-1..slice-7` receipts under `evidence/phase-9-legacy-seams-cleanup/`; model records `EV-P9-SLICE-2..7`.
