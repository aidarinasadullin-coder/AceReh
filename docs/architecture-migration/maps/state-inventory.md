...
| `ST-006` | Climate persisted inputs `SelectedCity`, `AirTemperature`, `WindSpeed`, `Humidity`, `SnowfallIntensity`, `SelectedZone`, `IsHighRequirements` | ClimateViewModel/IClimateData seam | CalculationContext.Climate; Results climate projection | climate UI, `Reset`, restore writers not fully enumerated | Results save/refresh, Thermal, Circuits | `OnDataChanged`, context update; counts unknown | `ClimateProjectData` fields except cold-five-day | `ProjectSession.ClimateState` | legacy | `ClimateViewModel.cs:389-409`; `ResultsViewModel.cs:1623-1634,986-994` | partial |
| `ST-007` | Climate reset/UI state `ColdFiveDayTemperature`, `HasUserModifications`, `SearchQuery` | ClimateViewModel | search/results UI | climate UI and `Reset` | climate UI; `SearchQuery` invokes search command | search clearing/search command; other effects unknown | not observed in `SaveCurrentProject` | `ProjectSession.ClimateState` or UI adapter decision | legacy | `ClimateViewModel.cs:75-92,389-409`; `ResultsViewModel.cs:1623-1634` | partial |
