...
| `CF-005` | climate edit | `tests/.../IntegrationTests/Hydraulics/DoubleCalculationPreventionTests.cs`; `UpdateFromClimateModule_TriggersSingleCalculate`; real Climate/Circuits VMs, mocked glycol/calculator/dirty | AirTemperature=-28; mocked GetProperties exactly 2 | ContextChanged=unknown; StateChanged=unknown; calculator=1 inferred from two GetProperties calls for one collector calculation; Results=unknown; dirty=unknown | partial | `FG-005` |
| `CF-006` | construction edit | `tests/.../ViewModels/ResultsViewModelOpenProjectTests.cs`; `GroundwaterLevelChange_AfterProjectLoad_UpdatesLambdaForBelowPipeLayers`; in-memory DTO/VMs | groundwater .5 yields LambdaB | ContextChanged=unknown; StateChanged=unknown; calculator=unknown; Results=unknown; dirty=unknown | partial | `FG-006` |
