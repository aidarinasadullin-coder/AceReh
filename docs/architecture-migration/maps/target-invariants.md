...
| `INV-003` | Construction values and layer collections SHALL have one writable canonical owner in `ProjectSession.ConstructionState`. | state-ownership, reactive, persistence | `ST-008..ST-011`; `RE-009`; `COV-PP` | Assert writer uniqueness and collection identity; prove every user-visible construction action crosses a public state/application mutation boundary and one logical-change completion boundary without intercepting ViewModel internals, including one action that groups multiple layer changes and non-user reset/load/save/reload paths. | unverified | Current model/ViewModel ownership is ambiguous. |
