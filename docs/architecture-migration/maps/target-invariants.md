...
| `INV-006` | Every migrated value SHALL have exactly one writable canonical owner; transitional dual-write paths SHALL be short-lived, compiling, and explicitly recorded as risk. | state-ownership | `ST-001..ST-027`; `maps/state-ownership.md` | Machine-check writer inventory per slice and reject more than one canonical writer at each phase gate. | verified | Closed 2026-09-03 (Phase 10): machine-checked writer inventory `SUMMARY: 8/8 checks PASS` across `ST-001..ST-027` (`evidence/phase-10-.../writer-inventory-output.txt`); `CalculationContext` `ST-020..ST-022` sites are exactly the four sanctioned DEC-001=A non-owning projection writers; zero ViewModel foreign-slice writes. Scope statement (recorded verbatim in the dossier): the global verdict covers the migrated write-set `ST-001..ST-027`. |
| `INV-007` | ViewModels SHALL be WPF adapters and SHALL NOT serve as shared canonical state stores or required mutation interception points after their slice migrates. | compile-time, di-runtime, state-ownership | `CTN-008..CTN-011`, `CTN-020`; `DRN-017..DRN-021`; `ST-003`, `ST-006..ST-019`, `ST-024..ST-027` | Inspect constructor contracts and writer inventory; run UI adapter characterization tests proving user actions use public state/application mutation boundaries and no future history recorder must intercept ViewModel setters, commands, or internal details. | verified | Closed 2026-09-03 (Phase 10): DI singleton constructor contracts inspected; `MutationBoundaryConsolidationTests` (36/36 with per-slice suites) prove user actions cross public state/application boundaries on public surfaces only; writer inventory WI-8 records zero foreign-slice ViewModel writes; no recorder interception required. Scope: migrated write-set `ST-001..ST-027`. |
...
## Phase 10 Reactive-Ownership-Multiplicity-Closure Overlay

`INV-010` is verified/implemented: the reactive view holds measured facts only
(structural QA adapted 2026-09-03 — every `RE-` row carries `slice-` provenance
and no counter cell is `unknown`; QA passes: 14 edges / 14 measured rows /
0 unmeasured / 0 orphans). The counting harness is sensitivity-proven (recorded
RED probe: an injected duplicate subscription fails 3/11 assertions) and
GREEN-stable across doubled load/reset cycles; the leak-hygiene slice is a
justified no-op (zero production subscription edits; combined suites 94/94);
full regression 2050/0/1-known-skip with `+18` tests exactly equal to this
phase's additions. The global `INV-006`/`INV-007` flips rest on the
machine-checked writer inventory (`SUMMARY: 8/8 checks PASS`, scope
`ST-001..ST-027`, DEC-001=A projection sites non-owning) plus the consolidated
`INV-016` mutation-boundary suite (36/36 on the plan-exact filter). Verifier
exemplar disposition: with no genuinely open invariant remaining, the owner
recorded disposition **B2** in-session (2026-09-03) — `verify-widget.mjs`
scenario copies now inject the synthetic `unverified` status locally
(`changed-unverified`/`added-survivor-unverified`/`removed-survivor-unverified`
unchanged in meaning; live model needs no open row); both verifier suites PASS
(model-v2 33 assertions, runtime-v2 PASS) after the amendment. Evidence:
`slice-1..slice-7` receipts under `evidence/phase-10-reactive-ownership-multiplicity-closure/`;
model records `EV-P10-*`.
