...
*   **Спецификация XLSX** — выгрузка спецификации закупки в книгу Excel (`.xlsx`): листы «Спецификация» и «Контуры» (наладка).

#### 📊 Спецификация XLSX: закупка и наладка

Меню «Отчёт PDF ▾ → Спецификация XLSX» (шаг «Результаты»; пункт активен, когда проект готов к формированию отчёта). Программа создаёт книгу Excel (`.xlsx`) с двумя листами.

**Лист «Спецификация»** — позиции закупки с артикулами РЕХАУ:

| Раздел | Что содержит |
| --- | --- |
| Труба | типоразмер RAUTHERM S, артикул, метраж (петли + подводки) |
| Резьбозажимные соединения (РЗС) | 2 шт на каждый контур (подача + обратка). Артикулы: 17×2 → 12506073002, 20×2 → 12506173002; для трубы 25×2,3 артикул в рознице не подтверждён — позиция выводится с примечанием «уточняется» |
| Коллекторы HKV | тип (HKV-D / IV 1¼" / IV 1½"), число контуров, артикул, количество по типоразмерам |
| Итого по проекту | число контуров, труба (м), мощность (кВт) |

**Лист «Контуры»** — наладка по контурам: № коллектора и контура, длина петли и подводки, площадь, мощность (Вт), расход (л/ч), потери Δp (кПа), обороты клапана, преднастройка (кПа).

> 💡 Числа записываются **числовыми ячейками**: их можно суммировать и фильтровать прямо в Excel; десятичный разделитель рисуется настройками Excel пользователя.
