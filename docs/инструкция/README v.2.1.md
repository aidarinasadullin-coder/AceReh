...
*   **λE** = **1.5 Вт/(м·К)** — теплопроводность слоя заливки трубы (Бетон плотный).
    (значение λ = 1.5 взято из `data/materials_db.json`, материал «Бетон плотный», id=5, λА = 1.5)
...
| **t_H** | −10 °C | Шаг 1 (Москва, T5Days092 ≥ −27 → колонка −10 °C) |
| **v_wind** | 1.7 м/с | Шаг 1 (`wind_avg_t_le_8` из `climate_db.json`)
...
> ℹ️ **Почему 9.94, а не 9.99:** Ручной расчёт выше использует `15^0.33 ≈ 2.466` (кубический корень из 15). В C# метод `Math.Pow(15, 0.33)` вычисляет вещественную степень: $15^{0.33} = e^{0.33 \times \ln 15} \approx 2.444$.  
> $2.26 \times 2.444 + 4.42 = \mathbf{9.94}$ — это корректный результат программы, не ошибка.
...
| **ρ_снега** | 900 кг/м³ | Константа `SnowDensity`, компактный снег |
...
*   **Тепловой поток вниз через слои изоляции ($q_D$):** **9.4 Вт/м²**  
    ...
    где $JHmü_{low} = T_{средняя} - t_G$, а $A, B, C, D, E$ — см. п. **4.1**.
    $$q_D = \frac{(43.0 - 10) \times 0.1539 + 20 \times 0.273 \times 0.111}{0.1539 \times 2.2215 \times (1.557 + 6.95 \times 0.273 \times 0.111)} \approx \mathbf{9.4\text{ Вт/м²}}$$
...
| Параметр | Значение | Источник |
|---|---|---|
| **R1** | 0.0533 м²·К/Вт | Шаг 2 (0.080 / 1.5) |
| **α** | 9.94 Вт/(м²·К) | Рассчитан в п.1 |

*(где **RD ≈ R2 = 2.2215 м²·К/Вт** — полное сопротивление слоёв под трубой, см. Шаг 2;
...
| **RFb** | 0.1539 м²·К/Вт | Рассчитан выше (R1 + 1/α) |
| **RD** | 2.2215 м²·К/Вт | Шаг 2, полное сопротивление слоёв под трубой |
| **λE** | 1.5 Вт/(м·К) | Шаг 2, Бетон плотный (id=5), `materials_db.json` |
...
| Параметр | Значение | Источник |
|---|---|---|
| **m** | 9.13 м⁻¹ | Рассчитан выше (параметр затухания) |
| **s** | 0.300 м | Шаг укладки, задан пользователем |
...
- **КПД ребра $\eta_R$** — отношение фактически переданного теплового потока к идеальному (когда вся поверхность имеет ту же температуру, что и труба). При $\eta_R = 1$ трубы уложены бесконечно часто; при $\eta_R = 0.642$ (наша конфигурация, шаг 300 мм) 35.8 % теплового потенциала теряется из-за дискретности раскладки.
...
|---|---|---|---|
| $A$ | $1 / \eta_R$ | $1 / 0.642 = 1.557$ | п.4, КПД ребра |
| $B$ | $1/RFb + 1/RD$ | $1/0.1539 + 1/2.2215 = 6.50 + 0.45 = 6.95$ | п.4 |
| $C$ | $\|t_H - t_G\|$ | $\|-10 - 10\| = 20$ K | Шаг 1 ($t_H$) + t_грунта |
| $D$ | $l_R / (\pi \times \lambda_R)$ | $0.300 / (\pi \times 0.35) = 0.273$ | шаг 300 мм, труба RAUTHERM S ($\lambda_R = 0.35$) |
| $E$ | $s / (d - s)$ | $2.0 / (20 - 2) = 0.111$ | труба 20×2.0 мм |

**Расчёт:**
$$
\begin{aligned}
JHmü &= \left[1.557 + \left(6.95 - \frac{20}{195.7 \times 0.1539 \times 2.2215}\right) \times 0.273 \times 0.111\right] \times 195.7 \times 0.1539 \\[4pt]
&= \left[1.557 + \left(6.95 - \frac{20}{66.94}\right) \times 0.0303\right] \times 30.12 \\[4pt]
&= \left[1.557 + (6.95 - 0.299) \times 0.0303\right] \times 30.12 \\[4pt]
&= \left[1.557 + 6.651 \times 0.0303\right] \times 30.12 \\[4pt]
&= \left[1.557 + 0.202\right] \times 30.12 = 1.759 \times 30.12 \approx \mathbf{53.0\ ^\circ\text{C}}
\end{aligned}
$$

**Физический смысл:** При шаге труб 300 мм и КПД ребра $\eta_R = 0.642$ теплоноситель должен быть на **53 °C горячее** окружающего воздуха ($-10\ ^\circ\text{C}$), чтобы обеспечить на поверхности мощность $q_{FB} = 195.7$ Вт/м². Это даёт среднюю температуру контура $T_{средняя} = 53.0 + (-10) = 43.0\ ^\circ\text{C}$.
...
> *Логика подбора:* $T_{подачи} = T_{средняя} + \Delta T_{рекомендуемый} / 2 = 43.0 + 15 / 2 = 43.0 + 7.5 = 50.5\text{ }^\circ\text{C}$. Программа округляет до целого — **51 °C**.  
...
| Параметр | Значение | Источник |
|---|---|---|
| **q_total** | 205.1 Вт/м² | PowerUp + q_D (п.3) |
| **c_p** | 4.04 кДж/(кг·К) | Шаг 1, гликоль 40 % (`glycol_data.json`) |
| **3.6** | 3600/1000 | Перевод кДж/(кг·К)→Дж/(кг·К) → кг/ч |
| **ΔT** | 16.0 K | T_подачи − T_обратки для скорректированной подачи 51 °C |
