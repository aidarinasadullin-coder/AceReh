...
- **Results — производная.** `ResultsViewModel` ничего не владеет:
  ...
  adapter, санкционирован), данные пересобираются из срезов. Готовность
  (`IsDataReady`) — реактивная производная: пересчитывается по событиям
  `Changed` слайсов, а не по навигации (ADR-011); тяжёлая гидратация
  контента остаётся на навигации.
