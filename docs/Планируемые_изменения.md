...
> **Примечание 2026-09-18 (волна 1 hardening-роадмапа, R-2026-09-18-03):**
> каталог `.omo/` выведен из репозитория (purge истории); упоминания путей
> `.omo/...` ниже — исторические, артефакты доступны в бандле
> `../ace-pre-surgery-2026-09-18.bundle`.
...
✅ Выполнено (проверено по коду 2026-09-13). Проверка реализована как пост-расчётная
в `ThermalResultValidator.Validate` (`src/Services/Thermal/ThermalResultValidator.cs:32–38`):
`T_обратки = 2×T_средняя − T_подачи < 0` → ошибка «Расчётная температура обратки (X °C)
отрицательна». Валидатор зарегистрирован в DI (`src/Configuration/ServiceCollectionExtensions.cs:290`)
и встроен в конвейер расчёта (`ThermalStateCoordinator.cs:49`, `ThermalViewModel.cs:397,466`).
Часть предложения «текст с советом, что делать» не реализована — закрыта идеей №2 из
[Идеи.md](Идеи.md) (подсказки «мощности не хватает»).
...
✅ Выполнено (проверено по коду 2026-09-13; зашитые 15.0 K и класс `ThermalParameters`
устранены последующими рефакторингами). Фактический ΔT считается в
`ThermalCalculator.cs:484` (`SupplyTemperature − ReturnTemperature`), гидравлика берёт его
из теплового результата: `CircuitsViewModel.cs:639` —
`deltaT = thermalResult?.DeltaT ?? (supplyTemperature − returnTemperature)`, расход контура —
`CircuitsCalculator.CalculateFlowRate` (`CircuitsCalculator.cs:42`). Контроль `ΔT ≤ 30 K` —
`ThermalResultValidator.cs:48` (плюс проверка положительности, строка 41). Проверка «при
ΔT > 30 K скорость потока не падает ниже MinVelocity» отдельно не делалась — скорость
контролируется гидравлическими валидаторами после расчёта по фактическому расходу.
