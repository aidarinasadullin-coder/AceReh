...
1. Запустите `SnowMeltingCalculator-v1.10.0-Setup.exe` (~68 MB)
...
# 3. Результат
# output\SnowMeltingCalculator-v1.10.0-Setup.exe
```
...
2. GitHub → **Actions → release → Run workflow** → версию можно оставить пустой
   ...
   совпадать с csproj
3. После зелёного прогона заберите артефакт `SnowMeltingCalculator-v1.10.0-Setup`
   из страницы запуска — он также автоматически уходит в папку выдачи на
   Google Drive (если настроен секрет `RCLONE_CONF_B64`); тег `v1.10.0`
